...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    {
        chassis_behaviour_mode = CHASSIS_SWING_GIMBAL_NO_MOVE;
    }
  ...
  else if (chassis_behaviour_mode == CHASSIS_SWING_GIMBAL_NO_MOVE)
  {
    chassis_move_mode->chassis_mode = CHASSIS_SWING;
  }
...
static void chassis_infantry_follow_gimbal_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    chassis_move_rc_to_vector->swing_flag = 0;

    //judge if swing
    //判断是否要摇摆
    if (chassis_move_rc_to_vector->chassis_RC->key.v & SWING_KEY)
  {
    chassis_move_rc_to_vector->swing_flag = 1;    
  }
}
	/**
  * @brief          when chassis behaviour mode is CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW, chassis control mode is speed control mode.
