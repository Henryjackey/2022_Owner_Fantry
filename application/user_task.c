...
void UserTask(void const *pvParameters) {
    ...
    while (1) {
        batter_percentage = get_cap_percent_message();
        bar.progress_bar_data_change = (batter_percentage > 0) ? batter_percentage : 0;

        // 刷新
        time = (time + 1) % 1000;  // 由于刷新延迟函数被删除，重置速度满一些
        if (time == 0) {
...
void UI_car_change() {
    ...
    // 后装甲板显示正常
    // 左装甲板是右装甲板
    // back_amour_attacked : 前装甲板受击打
    // get_front_amour_attacked: 左装甲板受击打
    car_front_armor_showing_attacked(&car, get_back_amour_attackek());
    car_back_armor_showing_attacked(&car, get_left_amour_attacked());
    car_left_armor_showing_attacked(&car, get_right_amour_attackek());
    car_right_armor_showing_attacked(&car, get_front_amour_attacked());
}
...
void ui_reset_car_front_armor_attacked_timer() {  // 这是左
    car_reset_right_armor_timer(&car);  // 因为绘制时左右颠倒了，所以这里要清空右计时器
}

void ui_reset_car_back_armor_attacked_timer() {  // 前装甲板
    car_reset_front_armor_timer(&car);
}

void ui_reset_car_left_armor_attacked_timer() {  // 后装甲板被攻击时回触发这个
    car_reset_back_armor_timer(&car);
}

void ui_reset_car_right_armor_attacked_timer() {  // 就是右
...
//        Line_Draw(&line_1, "901", UI_Graph_ADD, 3, UI_Color_Yellow, 2, 930, 450, 990, 450);
        ...
        UI_ReFresh(1, line_2);
        UI_ReFresh(2, line_5, line_8);
        UI_ReFresh(2, line_11, line_12);
...
//						Line_Draw(&line_1, "901", UI_Graph_ADD, 3, UI_Color_Yellow, 2, 930, 450, 990, 450);
        ...
        Line_Draw(&line_3, "902", UI_Graph_Change, 3, UI_Color_Orange, 2, 930, 467, 990, 467);
