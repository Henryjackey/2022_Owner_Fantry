...
void get_motor_measure_ecd(motor_measure_t *motor_measure, uint8_t data[8]){
    ...
    else if (motor_measure->ecd - motor_measure->last_ecd < -8191/2)
    {
		...
		if (motor_measure->ecd_count == FULL_COUNT)
    {
        motor_measure->ecd_count = -(FULL_COUNT - 1);
    }
    else if (motor_measure->ecd_count == -FULL_COUNT)
    {
        motor_measure->ecd_count = FULL_COUNT - 1;
    }
}
...
4:yaw云台电机 6020电机; 5:pitch云台电机 6020电机; 6:拨弹电机 2006电机*; 7:left fric 3508motor; 8:right fric 3508motor*/
...
/**
  * @brief         send FRIC control current of motor(0x201, 0x202)
  */
...
/**
  * @brief          send capacitance message
  */
/**
  * @brief          发送电容数据
  */
...
/**
  * @brief         return fric motor message
  * @param[in]     i is in the range of [1,2]
  */
...
/**
  * @brief         get the present capacitance message
  */
int16_t get_cap_percent_message()
{
	  return (int16_t) ((cap_measure.CapVot - 18)/(cap_measure.InputVot-18) * 100);
}
