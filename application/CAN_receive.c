...
void CAN_CMD_CAP(float power, float buffer)
{
  ...
  cap_tx_measure.DLC = 0x08;
	...
	cap_can_send_data[2] = (uint16_t)buffer >> 8;
	cap_can_send_data[3] = (uint16_t)buffer;
...
int16_t get_cap_percent()
{
	return (int16_t) ((cap_measure.CapVot - 5)/(cap_measure.InputVot-5) * 100);
}