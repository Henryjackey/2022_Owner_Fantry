...
void lost_wheel_current_allowcate(chassis_move_t * current_allowcate);
...
static void chassis_init(chassis_move_t *chassis_move_init)
{
  ...
  //初始化角度PID
	PID_init(&chassis_move_init->cap_voltage_pid, PID_POSITION, voltage_pid, 10000, 8000);
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
  ...
  vx_set_channel = vx_channel /660.0f*((float)max_power_limit/40.0f);
  vy_set_channel = -vy_channel /660.0f*((float)max_power_limit/80.0f);
	...
	if(!(chassis_move_rc_to_vector ->chassis_RC ->key.v & KEY_PRESSED_OFFSET_CTRL))
	{
		...
		else
		{
			...
			else
			{
				if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_FRONT_KEY)
				{
					vx_set_channel = ((float)max_power_limit/40.0);
				}
				else if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_BACK_KEY)
				{
					vx_set_channel = -((float)max_power_limit/40.0);
				...
				if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_LEFT_KEY)
				{
					vy_set_channel = ((float)max_power_limit/80.0);
				}
				else if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_RIGHT_KEY)
				{
					vy_set_channel = -((float)max_power_limit/80.0);
  ...
  if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW)
  {
    if (chassis_move_control->swing_flag)
    {
      ...
      //set control relative angle  set-point
			...
			{
				chassis_move_control->wz_set = chassis_power/9.0;
			}
      //speed limit
      //速度限幅
...
//     	if(cap_state)
//			{
//				chassis_move_control->wz_set = chassis_power/9.0*1.4f;
//			}
//			else
//			{
//				chassis_move_control->wz_set = chassis_power/9.0;
...
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
  ...
  for (i = 0; i < 4; i++)
  {
	...
	lost_wheel_current_allowcate(chassis_move_control_loop);
  //功率控制
...
void lost_wheel_current_allowcate(chassis_move_t * current_allowcate)
{
	...
	{
		if(current_allowcate->lost_wheel_flag==0)
		{	
			current_allowcate->motor_speed_pid[0].out =0;
			current_allowcate->motor_speed_pid[1].out =0;
		}
		else if(current_allowcate->lost_wheel_flag==1)
		{
			current_allowcate->motor_speed_pid[0].out =0;
			current_allowcate->motor_speed_pid[1].out =0;
		}
		else if(current_allowcate->lost_wheel_flag==2)
		{
			current_allowcate->motor_speed_pid[2].out =0;
			current_allowcate->motor_speed_pid[0].out =0;		
		}
	}
	else if(toe_is_error(CHASSIS_MOTOR2_TOE))
	{
		...
		{	
			current_allowcate->motor_speed_pid[0].out =0;
			current_allowcate->motor_speed_pid[1].out =0;
		//	current_allowcate->lost_wheel_flag=(current_allowcate->lost_wheel_flag+1)%3;
		}
		else if(current_allowcate->lost_wheel_flag==1)
		{
			current_allowcate->motor_speed_pid[0].out =0;
			current_allowcate->motor_speed_pid[3].out =0;
		//	current_allowcate->lost_wheel_flag=(current_allowcate->lost_wheel_flag+1)%3;
		}
		else if(current_allowcate->lost_wheel_flag==2)
		{
...
uint8_t get_cap_state()  // return 0 if cap is not using, 1 using
{
	return cap_state;
}
