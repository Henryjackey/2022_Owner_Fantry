...
void chassis_power_control(chassis_move_t *chassis_power_control)
{
    ...
    else
    {
				if(!toe_is_error(CAP_TOE))
				{
					...
					if(cap_state)
					{
						PID_calc(&chassis_power_control->cap_voltage_pid,cap_measure.CapVot,8);
					}
					else
					{
						PID_calc(&chassis_power_control->cap_voltage_pid,cap_measure.CapVot,battery_voltage-1.5);
					}
					power_total_current_limit = (max_power_limit - chassis_power_control->buffer_pid.out)* 6000.00/cap_measure.CapVot;
					if(cap_measure.CapVot>5)
					{
